...
p.to_csv("C:/Users/USUARIO/Documents/model/current6.csv",index=False)
print("p se escribio correctamente")
horaFinal=datetime.datetime.now()
print(horaFinal)
print(horaFinal-horaInicio)

p=pd.read_csv("C:/Users/USUARIO/Documents/model/current.csv")
import ast
...
def writeCSVFinalBatch():
    ...
    path='C:/Users/USUARIO/Documents/model/predicciones/prediccionFinal.csv'
...
def writeCSV(number):
    ...
    #finalDate
    df=pd.DataFrame({'fecha':finalDate,'prediccion':finalPrediction,'real':last_batch,'precios':precios_semana})
    path='C:/Users/USUARIO/Documents/model/predicciones/prediccion'+str(number)+'.csv'       
    df.to_csv(path,index=False)        
    return df
df=writeCSV(1)